...
<body class="font-sans antialiased h-screen flex overflow-hidden bg-gray-50 dark:bg-slate-900 text-slate-800 dark:text-slate-100" x-data="{ sidebarOpen: false }">
    ...
    <aside 
        ...
        <!-- Sidebar Links -->
        <div class="flex-1 overflow-y-auto py-6 px-4 space-y-5">
            @php
                $sidebarSections = [
                    [
                        'title' => __('Overview'),
                        'items' => [
                            ['route' => 'admin.dashboard', 'active' => 'admin.dashboard', 'label' => __('Dashboard'), 'icon' => 'DB'],
                        ],
                    ],
                    [
                        'title' => __('Academics'),
                        'items' => [
                            ['route' => 'admin.courses.index', 'active' => 'admin.courses.*', 'label' => __('Courses'), 'icon' => 'CR'],
                            ['route' => 'admin.live-sessions.index', 'active' => 'admin.live-sessions.*', 'label' => __('Live Sessions'), 'icon' => 'LS'],
                            ['route' => 'admin.quizzes.index', 'active' => 'admin.quizzes.*', 'label' => __('Quizzes'), 'icon' => 'QZ'],
                            ['route' => 'admin.questions.index', 'active' => 'admin.questions.*', 'label' => __('Questions'), 'icon' => 'QS'],
                            ['route' => 'admin.analytics.questions', 'active' => 'admin.analytics.questions', 'label' => __('Question Analytics'), 'icon' => 'AN'],
                        ],
                    ],
                    [
                        'title' => __('Students & Billing'),
                        'items' => [
                            ['route' => 'admin.students.index', 'active' => 'admin.students.*', 'label' => __('Students'), 'icon' => 'ST'],
                            ['route' => 'admin.device-requests.index', 'active' => 'admin.device-requests.*', 'label' => __('Device Requests'), 'icon' => 'DV'],
                            ['route' => 'admin.payments.index', 'active' => 'admin.payments.*', 'label' => __('Payments'), 'icon' => 'PM'],
                            ['route' => 'admin.promo-codes.index', 'active' => 'admin.promo-codes.*', 'label' => __('Promo Codes'), 'icon' => 'PC'],
                            ['route' => 'admin.certificates.index', 'active' => 'admin.certificates.*', 'label' => __('Certificates'), 'icon' => 'CT'],
                        ],
                    ],
                    [
                        'title' => __('Community'),
                        'items' => [
                            ['route' => 'admin.forum.index', 'active' => 'admin.forum.*', 'label' => __('Forum'), 'icon' => 'FR'],
                            ['route' => 'admin.games.index', 'active' => 'admin.games.*', 'label' => __('Games'), 'icon' => 'GM'],
                            ['route' => 'admin.email-campaigns.index', 'active' => 'admin.email-campaigns.*', 'label' => __('Email Campaigns'), 'icon' => 'EM'],
                        ],
                    ],
                    [
                        'title' => __('Website'),
                        'items' => [
                            ['route' => 'admin.testimonials.index', 'active' => 'admin.testimonials.*', 'label' => __('Testimonials'), 'icon' => 'TS'],
                            ['route' => 'admin.promo-videos.index', 'active' => 'admin.promo-videos.*', 'label' => __('Promo Videos'), 'icon' => 'PV'],
                        ],
                    ],
                    [
                        'title' => __('Settings'),
                        'items' => [
                            ['route' => 'admin.settings.index', 'active' => 'admin.settings.index', 'label' => __('Settings Home'), 'icon' => 'SH'],
                            ['route' => 'admin.settings.general', 'active' => 'admin.settings.general', 'label' => __('General'), 'icon' => 'GN'],
                            ['route' => 'admin.settings.security', 'active' => 'admin.settings.security*', 'label' => __('Security'), 'icon' => 'SC'],
                            ['route' => 'admin.settings.telegram', 'active' => 'admin.settings.telegram', 'label' => __('Telegram Bot'), 'icon' => 'TG'],
                            ['route' => 'admin.settings.payment', 'active' => 'admin.settings.payment', 'label' => __('Payment Settings'), 'icon' => 'PY'],
                            ['route' => 'admin.settings.points', 'active' => 'admin.settings.points', 'label' => __('Points & Rewards'), 'icon' => 'PT'],
                            ['route' => 'admin.settings.battle', 'active' => 'admin.settings.battle', 'label' => __('Battle Settings'), 'icon' => 'BT'],
                        ],
                    ],
                ];
            @endphp
            ...
            @foreach($sidebarSections as $section)
                <div class="space-y-1.5">
                    <p class="px-2 text-xs font-bold text-slate-500 uppercase tracking-wider">{{ $section['title'] }}</p>
                    @foreach($section['items'] as $item)
                        <a href="{{ route($item['route']) }}" class="admin-sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800 transition-all group {{ request()->routeIs($item['active']) ? 'active' : '' }}">
                            <span class="w-6 h-6 rounded-md bg-slate-800 border border-slate-700 flex items-center justify-center text-[10px] font-bold text-slate-300 group-hover:text-white group-hover:border-slate-500 transition-colors">{{ $item['icon'] }}</span>
        ...
        <div class="flex-1 overflow-y-auto py-6 px-4 space-y-1">
            ...
            <p class="px-2 text-xs font-bold text-slate-500 uppercase tracking-wider mb-2 mt-6">{{ __('الصفحة الرئيسية') }}</p>

            <a href="{{ route('admin.testimonials.index') }}" class="admin-sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800 transition-all group {{ request()->routeIs('admin.testimonials.*') ? 'active' : '' }}">
                <span class="text-xl group-hover:scale-110 transition-transform">⭐</span>
                <span class="font-medium text-sm">{{ __('آراء الطلاب') }}</span>
            </a>

            <a href="{{ route('admin.promo-videos.index') }}" class="admin-sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800 transition-all group {{ request()->routeIs('admin.promo-videos.*') ? 'active' : '' }}">
                <span class="text-xl group-hover:scale-110 transition-transform">🎬</span>
                <span class="font-medium text-sm">{{ __('عينة الشروحات') }}</span>
            </a>
