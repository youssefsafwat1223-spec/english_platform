...
@auth
    @if(auth()->user()->is_student)
        <a href="https://wa.me/966537191862" target="_blank"
           class="hidden lg:flex fixed lg:bottom-8 lg:right-8 z-[105] lg:w-14 lg:h-14 rounded-full bg-[#25D366] text-white items-center justify-center shadow-2xl hover:scale-110 hover:shadow-[#25D366]/50 transition-all duration-300 group"
...
@auth
    @if(auth()->user()->is_student && !request()->routeIs('student.quizzes.start'))
        ...
        <div class="fixed left-4 right-4 sm:left-8 sm:right-8 transition-all duration-500"
            ...
            {{-- WhatsApp Dome — sits above navbar, centered --}}
            <div class="absolute left-1/2 -translate-x-1/2 -top-[30px] z-[101] flex items-end justify-center"
                 style="width: 72px; height: 50px;">
                {{-- Dome background shape --}}
                <div class="absolute inset-0 overflow-hidden">
                    <div class="w-full h-full bg-white/80 dark:bg-[#020617]/80 backdrop-blur-[30px] saturate-[1.8] border border-white/60 dark:border-white/10 border-b-0"
                         style="border-radius: 36px 36px 0 0;"></div>
                </div>
                {{-- WhatsApp Button --}}
                <a href="https://wa.me/966537191862" target="_blank" rel="noopener noreferrer"
                   class="relative -mb-[10px] z-10 w-[52px] h-[52px] flex items-center justify-center rounded-full bg-gradient-to-br from-[#25D366] to-[#128C7E] shadow-[0_4px_20px_rgba(37,211,102,0.4)] hover:shadow-[0_6px_28px_rgba(37,211,102,0.6)] active:scale-90 transition-all duration-300">
