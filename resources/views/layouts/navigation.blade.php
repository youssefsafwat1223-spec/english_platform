...
<nav x-data="{ open: false, userOpen: false, scrolled: false, gtLoaded: false }"
     ...
     class="fixed top-4 left-4 right-4 lg:left-8 lg:right-8 z-50 transition-shadow duration-300 ease-out glass-card will-change-transform bg-white/95 dark:bg-[#020617]/95"
        ...
        <div class="flex justify-between h-16 lg:h-20 items-center">
            ...
            <div class="flex items-center gap-8">
                <a href="{{ route('home') }}" class="flex items-center gap-3 group">
                    <img src="{{ asset('images/logo.png') }}" alt="Simple English Logo" class="h-10 w-auto object-contain transition-transform duration-300 group-hover:scale-110">
                    <span class="font-sans text-xl font-bold tracking-tight text-slate-900 dark:text-white whitespace-nowrap group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">
                        Simple English
                    </span>
                </a>
                ...
                <div class="hidden lg:flex items-center gap-1">
                    ...
                    @auth
                        ...
                        @elseif(auth()->user()->is_admin)
                            ...
                            <!-- Language Switcher -->
                            <a href="{{ route('switch-lang', app()->getLocale() == 'en' ? 'ar' : 'en') }}"
                               class="nav-link flex items-center gap-1 font-medium"
                               title="Switch Language">
                                <span class="text-lg">{{ app()->getLocale() == 'en' ? '🇪🇬' : '🇺🇸' }}</span>
                                <span class="hidden xl:inline">{{ app()->getLocale() == 'en' ? 'All' : 'English' }}</span>
                            </a>
            ...
            <div class="flex items-center gap-1 sm:gap-3">
                ...
                <button type="button"
                        onclick="toggleTheme()"
                        class="relative w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center transition-all duration-300 hover:bg-gray-100 dark:hover:bg-white/5 border border-transparent hover:border-gray-200 dark:hover:border-white/10"
                ...
                <a href="{{ route('locale.switch', app()->getLocale() === 'ar' ? 'en' : 'ar') }}"
                   class="relative w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center transition-all duration-300 hover:bg-gray-100 dark:hover:bg-white/5 border border-transparent hover:border-gray-200 dark:hover:border-white/10 text-xs sm:text-sm font-bold"
                ...
                <button @click="open = !open"
                        class="lg:hidden inline-flex items-center justify-center w-10 h-10 rounded-xl transition-colors hover:bg-gray-100 dark:hover:bg-white/5"
