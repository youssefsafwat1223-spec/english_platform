    ...
    <div class="hidden lg:flex lg:w-1/2 relative overflow-hidden bg-slate-900">
        ...
        <div class="relative z-10 flex flex-col justify-between p-16 w-full h-full">
            ...
            <div class="flex items-center gap-8 text-sm font-bold text-slate-500 uppercase tracking-widest">
                <span>{{ \App\Models\Course::count() ?? '50+' }} {{ __('Courses') }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="w-full lg:w-1/2 flex items-center justify-center p-8 sm:p-12 lg:p-16 relative z-10">
        ...
        <div class="w-full max-w-md animate-fade-in-up">
            ...
            <div class="mb-10 text-center lg:text-left">
                <h1 class="text-4xl font-black text-slate-900 dark:text-white tracking-tight mb-3">{{ __('Log In') }}</h1>
                <p class="text-slate-500 dark:text-slate-400 font-medium">
                    {{ __('Don\'t have an account?') }} 
                    <a href="{{ route('register') }}" class="text-primary-600 dark:text-primary-400 font-bold hover:underline underline-offset-4 transition-all">{{ __('Create Account') }}</a>
                </p>
            </div>

            @if (session('status'))
                <div class="mb-6 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-600 dark:text-emerald-400 px-4 py-3 rounded-xl text-sm font-bold flex items-center gap-2">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-6" x-data="{ loading: false }" @submit="loading = true">
                ...
                <div x-data="{ show: false }">
                    <div class="flex items-center justify-between mb-2">
                        <label for="password" class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('Password') }}</label>
                        <a href="{{ route('password.request') }}" class="text-sm font-bold text-primary-600 dark:text-primary-400 hover:underline transition-all">{{ __('Forgot your password?') }}</a>
                    </div>
                    <div class="relative">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                ...
                <div class="flex items-center">
                    <label class="flex items-center gap-3 cursor-pointer group">
                        <input type="checkbox" name="remember" id="remember" class="w-5 h-5 rounded border-2 border-slate-300 dark:border-white/20 text-primary-500 bg-transparent focus:ring-primary-500 focus:ring-offset-0 transition-colors cursor-pointer">
                        <span class="text-sm font-medium text-slate-600 dark:text-slate-400 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{{ __('Remember me') }}</span>
                    </label>
                </div>
