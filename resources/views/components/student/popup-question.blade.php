<div x-data="popupQuestionManager()" x-init="initTimer()" x-cloak>
    ...
    <div x-show="showModal" 
        ...
        <div class="bg-white dark:bg-slate-900 rounded-[1.5rem] shadow-2xl w-full max-w-md overflow-hidden transform transition-all duration-300 pointer-events-auto"
             @click.away="!isSubmitting && !showResult && shakeModal()"
             x-ref="modalBox"
             x-transition:enter="ease-out duration-300"
            ...
            {{-- Header --}}
            <div class="px-6 py-4 border-b border-slate-100 dark:border-white/10 bg-gradient-to-r from-primary-50 to-primary-100/50 dark:from-primary-500/10 dark:to-accent-500/10 flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-primary-100 dark:bg-primary-500/20 text-primary-600 dark:text-primary-400 flex items-center justify-center shrink-0">
                    <svg class="w-5 h-5 animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.54-.994 1.093m0 3h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                </div>
                <div>
                    <h3 class="font-bold text-slate-900 dark:text-white">{{ app()->getLocale() === 'ar' ? 'سؤال سريع!' : 'Quick Question!' }}</h3>
                    <p class="text-xs font-medium text-slate-500 dark:text-slate-400">{{ app()->getLocale() === 'ar' ? 'نشّط ذهنك قبل ما تكمل.' : 'Refresh your mind before continuing.' }}</p>
                </div>
            </div>
            ...
            <div class="p-6">
                ...
                <div x-show="isLoading" class="flex flex-col items-center justify-center py-6 gap-3">
                    ...
                    <span class="text-sm font-bold text-slate-500">{{ app()->getLocale() === 'ar' ? 'جاري تحميل السؤال...' : 'Loading question...' }}</span>
                </div>

                {{-- Question Content --}}
                <div x-show="!isLoading && questionText">
                    <p class="text-lg font-bold text-slate-800 dark:text-slate-200 mb-6 leading-relaxed" x-text="questionText"></p>

                    <div class="space-y-3">
                        <template x-for="(option, index) in options" :key="index">
                            <label class="flex items-center gap-3 p-4 rounded-xl border-2 cursor-pointer transition-all duration-200 group"
                                   :class="{
                                ...
                                <input type="radio" x-model="selectedAnswer" :value="option" class="hidden" :disabled="isSubmitting || showResult">
                                <span class="text-sm font-bold text-slate-700 dark:text-slate-300" x-text="option"></span>
                                
                                {{-- Icon for result feedback --}}
                                <div class="ml-auto flex items-center" x-show="showResult">
                                    <svg x-show="resultCorrect && selectedAnswer === option" class="w-5 h-5 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                    <svg x-show="!resultCorrect && selectedAnswer === option" class="w-5 h-5 text-rose-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                    <svg x-show="!resultCorrect && option === correctOptionText" class="w-5 h-5 text-emerald-500/70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                </div>
                            </label>
                        </template>
                    </div>
                    ...
                    <div x-show="showResult" 
                         x-collapse 
                         class="mt-4 p-4 rounded-xl flex items-start gap-3"
                        ...
                        <svg x-show="resultCorrect" class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        <svg x-show="!resultCorrect" class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        <div class="text-sm font-bold">
                            <p x-text="resultCorrect ? '{{ app()->getLocale() === 'ar' ? 'إجابة صحيحة! أحسنت.' : 'Correct answer! Great job.' }}' : '{{ app()->getLocale() === 'ar' ? 'إجابة خاطئة. حاول التركيز أكثر.' : 'Incorrect answer. Try to focus more!' }}'"></p>
                        </div>
                    </div>
                </div>
                ...
                <div class="mt-6">
                    <button type="button" 
                        ...
                        <span x-show="!isSubmitting">{{ app()->getLocale() === 'ar' ? 'تأكيد الإجابة' : 'Submit Answer' }}</span>
                    ...
                    <button type="button" 
                        ...
                        <svg class="w-4 h-4 mr-1 rtl:rotate-180" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                    </button>
                </div>
            </div>
        </div>
