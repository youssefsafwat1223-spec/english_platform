        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10" data-aos="fade-down">
            <div>
                <a href="{{ route('student.courses.show', $course) }}" class="inline-flex items-center text-sm font-bold text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-white transition-colors mb-4 group">
                    ...
                    {{ __('ui.checkout.back_to_course') }}
                </a>
                <h1 class="text-4xl md:text-5xl font-black text-slate-900 dark:text-white tracking-tight drop-shadow-sm dark:drop-shadow-lg">{{ __('ui.checkout.title') }}</h1>
            </div>
        ...
        @if(session('error') || $errors->has('payment'))
            <div class="mb-8 rounded-2xl border border-rose-200 bg-rose-50 px-5 py-4 text-rose-800 shadow-sm dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-200" data-aos="fade-up">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-black">{{ __('ui.checkout.payment_error_title') }}</p>
                        ...
                        <h3 class="text-xl font-black text-slate-900 dark:text-white flex items-center gap-3">
                            <span class="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center text-white shadow-lg">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7h16M4 11h16M8 15h.01M12 15h.01M16 15h.01M6 19h12a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2Z" />
                                </svg>
                            </span>
                    ...
                    <div class="p-8 space-y-6">
                        <form action="{{ route('student.courses.payment', $course) }}" method="POST" id="paymentForm" x-data="{ loading: false }" x-init="window.addEventListener('pageshow', () => { loading = false })" @submit="loading = true">
                            ...
                            <div class="space-y-4 mb-4">
                                @if(isset($promoCode))
                                    ...
                                    <div class="p-4 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl flex items-center justify-between">
                                        <div>
                                            <p class="text-sm font-bold text-emerald-800 dark:text-emerald-400">{{ __('ui.checkout.promo_applied') }}</p>
                                            <p class="text-xs text-emerald-600 dark:text-emerald-500 font-mono mt-1">{{ $promoCode->code }}</p>
                                        </div>
                                        <a href="{{ route('student.courses.remove-discount', $course) }}" class="text-xs font-bold text-rose-500 hover:text-rose-600 underline px-2">{{ __('ui.checkout.remove') }}</a>
                                    </div>
                                    <input type="hidden" name="promo_code_id" value="{{ $promoCode->id }}">
                                @elseif(auth()->user()->has_referral_discount)
                                    <div class="p-4 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl flex items-center justify-between">
                                        <div>
                                            <p class="text-sm font-bold text-emerald-800 dark:text-emerald-400">{{ __('ui.checkout.referral_discount_applied') }}</p>
                                            <p class="text-xs text-emerald-600 dark:text-emerald-500 font-medium mt-1">{{ session('referral_code', __('ui.checkout.referral_discount_account')) }}</p>
                                        </div>
                                        <a href="{{ route('student.courses.remove-discount', $course) }}" class="text-xs font-bold text-rose-500 hover:text-rose-600 underline px-2">{{ __('ui.checkout.remove') }}</a>
                                    </div>
                                @else
                                    <label class="block text-sm font-bold text-slate-700 dark:text-slate-300">{{ __('ui.checkout.referral_code_label') }}</label>
                            ...
                            <div class="mb-8 p-4 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl">
                                <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">{{ __('ui.checkout.promo_code_label') }}</label>
                                <div class="flex gap-2">
                                    <input type="text" id="promo_code_input" class="w-full bg-white dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl py-3 px-4 text-sm font-mono uppercase text-slate-900 dark:text-white focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-colors" placeholder="{{ __('ui.checkout.example') }} SUMMER50">
                                    <button type="button" onclick="applyPromoCode()" class="bg-slate-800 dark:bg-white text-white dark:text-slate-900 px-4 py-2 rounded-xl text-sm font-bold hover:bg-slate-700 dark:hover:bg-slate-200 whitespace-nowrap">{{ __('ui.checkout.apply') }}</button>
                                </div>
                                <script>
                ...
                <div class="sticky top-28" data-aos="fade-left" data-aos-delay="100">
                    ...
                    <div class="relative bg-white/80 dark:bg-[#0f172a]/80 backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-[2rem] p-1 shadow-2xl shadow-slate-200/50 dark:shadow-black/50 overflow-hidden group">
                        ...
                        <div class="bg-slate-50 dark:bg-[#020617] rounded-[1.8rem] p-8 relative z-10 border border-slate-100 dark:border-white/5">
                            ...
                            <h3 class="text-xl font-black text-slate-900 dark:text-white mb-6 tracking-wider">{{ __('ui.checkout.order_summary') }}</h3>
                            ...
                            <div class="flex gap-4 items-center mb-8 bg-white dark:bg-white/5 rounded-xl p-3 border border-slate-200 dark:border-white/5 shadow-sm">
                                ...
                                @else
                                    <div class="w-20 h-20 rounded-lg bg-gradient-to-br from-slate-200 to-slate-300 dark:from-slate-800 dark:to-slate-900 flex items-center justify-center shrink-0 shadow-sm border border-slate-200 dark:border-white/5 text-slate-600 dark:text-slate-300">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500 mt-1 font-medium">{{ $course->level ?? __('ui.checkout.course_level_fallback') }}</p>
                                </div>
                            </div>
                                ...
                                @if($discount > 0)
                                    <div class="flex justify-between items-center text-emerald-600 dark:text-emerald-400 font-bold bg-emerald-50 dark:bg-emerald-500/10 p-2 rounded-lg -mx-2 px-2 border border-emerald-100 dark:border-emerald-500/20">
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            @if(isset($promoCode))
                                                {{ __('ui.checkout.coupon_discount') }}
                                            @else
