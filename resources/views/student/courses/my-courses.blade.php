        ...
        <form method="GET" action="{{ route('student.courses.my-courses') }}" class="glass-card mb-8" data-aos="fade-up">
            <div class="glass-card-body grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="md:col-span-2">
                    <label for="q" class="block text-sm font-semibold mb-2" style="color: var(--color-text);">Search</label>
                    <div class="relative">
                        ...
                        <input id="q" name="q" type="text" class="input-glass pl-12" value="{{ request('q') }}" placeholder="Search your courses">
                    </div>
                </div>
                <div>
                    <label for="status" class="block text-sm font-semibold mb-2" style="color: var(--color-text);">Status</label>
            ...
            <div class="glass-card-footer flex items-center justify-between">
                <button type="submit" class="btn-primary ripple-btn">Apply</button>
                @if(request()->query())
                    <a href="{{ route('student.courses.my-courses') }}" class="text-sm hover:text-primary-500 transition-colors" style="color: var(--color-text-muted);">Clear Filters</a>
                @endif
            </div>
        </form>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <a href="{{ route('student.courses.learn', $enrollment->course) }}" class="block">
                        ...
                        <div class="glass-card-body">
                            ...
                            <div class="mb-4">
                                ...
                                <div class="text-xs mt-2" style="color: var(--color-text-muted);">Last activity: {{ $lastAccessed }}</div>
                            </div>
                        </div>
                    </a>
                    ...
                    <div class="glass-card-body pt-0">
                        <div class="flex justify-between items-center pt-4" style="border-top: 1px solid var(--glass-border);">
                            @if($enrollment->is_completed)
                                ...
                                <a href="{{ route('student.courses.certificate.info', $enrollment->course) }}" class="btn-primary btn-sm">🎓 Certificate</a>
                            @else
                                <a href="{{ route('student.courses.learn', $enrollment->course) }}" class="btn-primary btn-sm ripple-btn">Continue Learning</a>
                            @endif
                        </div>
                    </div>
                </div>
            @empty
                <div class="col-span-3 text-center py-16" data-aos="fade-up">
                    ...
                    <p class="text-lg font-medium mb-2" style="color: var(--color-text);">No courses yet</p>
                    <p class="mb-6" style="color: var(--color-text-muted);">Start your learning journey today!</p>
                    <a href="{{ route('student.courses.index') }}" class="btn-primary ripple-btn">Browse Courses</a>
                </div>
            @endforelse
        </div>
