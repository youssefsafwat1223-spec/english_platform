...
@section('title', __('تعديل رأي طالب'))
@section('content')
<div class="py-12 relative overflow-hidden">
    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 relative z-10">
        <div class="mb-8" data-aos="fade-down">
            ...
            <h1 class="text-3xl font-extrabold mt-4"><span class="text-gradient">{{ __('تعديل رأي طالب') }}</span></h1>
        </div>
            ...
            <form action="{{ route('admin.testimonials.update', $testimonial) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="content" rows="4" required
                        class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl py-3 px-4 font-semibold focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-colors" style="color: var(--color-text);">{{ old('content', $testimonial->content) }}</textarea>
                ...
                <div>
                    ...
                    @if($testimonial->avatar)
                        <div class="flex items-center gap-4 mb-3">
                            <img src="{{ Storage::url($testimonial->avatar) }}" class="w-16 h-16 rounded-full object-cover border-2 border-primary-500/30" alt="{{ $testimonial->name }}">
                ...
                <div>
                    ...
                    <select name="rating" required
                        class="w-full bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-white/10 rounded-xl py-3 px-4 font-semibold focus:border-primary-500 focus:ring-1 focus:ring-primary-500 transition-colors" style="color: var(--color-text);">
                        @for($i = 5; $i >= 1; $i--)
                            <option value="{{ $i }}" {{ old('rating', $testimonial->rating) == $i ? 'selected' : '' }}>{{ $i }} {{ __('نجوم') }} {{ str_repeat('★', $i) }}</option>
                        @endfor
                    </select>
