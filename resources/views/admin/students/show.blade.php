        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                <div class="glass-card overflow-hidden" data-aos="fade-up">
                    <div class="glass-card-header">
                        <h3 class="font-bold" style="color: var(--color-text);">{{ __('Student Information') }}</h3>
                    </div>
                    <div class="glass-card-body">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div><div class="text-xs" style="color: var(--color-text-muted);">{{ __('Last Active') }}</div><div class="font-bold" style="color: var(--color-text);">{{ $student->last_activity_at ? $student->last_activity_at->diffForHumans() : __('Never') }}</div></div>
                ...
                <div class="glass-card overflow-hidden" data-aos="fade-up">
                    <div class="glass-card-header">
                        <h3 class="font-bold" style="color: var(--color-text);">{{ __('Performance Overview') }}</h3>
                    </div>
                    <div class="glass-card-body">
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
                            ...
                            @foreach($perfStats as $ps)
                                <div>
                                    <div class="text-2xl font-extrabold {{ $ps['c'] }}">{{ $ps['v'] }}</div>
                                    <div class="text-xs" style="color: var(--color-text-muted);">{{ $ps['l'] }}</div>
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="glass-card overflow-hidden" data-aos="fade-up">
                    <div class="glass-card-header flex justify-between items-center">
                        <h3 class="font-bold" style="color: var(--color-text);">{{ __('Recent Enrollments') }}</h3>
                        <a href="{{ route('admin.students.enrollments', $student) }}" class="text-primary-500 font-bold text-sm hover:underline">{{ __('View All') }}</a>
                    </div>
                    <div class="glass-card-body divide-y" style="border-color: var(--color-border);">
                        @forelse($student->enrollments()->latest()->take(5)->get() as $enrollment)
                            <div class="flex justify-between items-center py-3 first:pt-0 last:pb-0">
                                <div>
                                    <div class="font-bold text-sm" style="color: var(--color-text);">{{ $enrollment->course?->title ?? __('Deleted course') }}</div>
                                    <div class="text-xs" style="color: var(--color-text-muted);">{{ $enrollment->created_at->diffForHumans() }}</div>
                                </div>
                                <div class="text-right">
                                    <div class="text-sm font-bold text-primary-500">{{ round($enrollment->progress_percentage) }}%</div>
                                    @if($enrollment->is_completed)
                                        <span class="badge-success text-[10px]">{{ __('Completed') }}</span>
                                    @endif
                                </div>
                            </div>
                        @empty
                            <p class="text-center py-4 text-sm" style="color: var(--color-text-muted);">{{ __('No enrollments') }}</p>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="glass-card overflow-hidden" data-aos="fade-up">
                    <div class="glass-card-header">
                        <h3 class="font-bold" style="color: var(--color-text);">{{ __('Recent Activity') }}</h3>
                    </div>
                    <div class="glass-card-body divide-y" style="border-color: var(--color-border);">
                        @forelse($student->pointsHistory()->latest()->take(10)->get() as $activity)
                            <div class="flex justify-between items-center py-2 first:pt-0 last:pb-0">
                                <div>
                                    <div class="font-bold text-sm" style="color: var(--color-text);">{{ $activity->description }}</div>
                                    <div class="text-xs" style="color: var(--color-text-muted);">{{ $activity->created_at->diffForHumans() }}</div>
                                </div>
                                <span class="font-bold text-emerald-500">+{{ $activity->points_earned }}</span>
                            </div>
                        @empty
                            <p class="text-center py-4 text-sm" style="color: var(--color-text-muted);">{{ __('No activity') }}</p>
                        @endforelse
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-6">
                <div class="glass-card overflow-hidden" data-aos="fade-left">
                    <div class="glass-card-header">
                        <h3 class="font-bold" style="color: var(--color-text);">{{ __('Actions') }}</h3>
                    </div>
                    <div class="glass-card-body space-y-2">
                        <form action="{{ route('admin.students.toggle-status', $student) }}" method="POST">
                            @csrf
                            <button type="submit" class="{{ $student->is_active ? 'btn-secondary' : 'btn-primary ripple-btn' }} w-full">
                                {{ $student->is_active ? __('Deactivate') : __('Activate') }}
                            </button>
                        </form>
                        <a href="{{ route('admin.students.enrollments', $student) }}" class="btn-secondary w-full text-center block">{{ __('View Enrollments') }}</a>
                        <a href="{{ route('admin.students.index') }}" class="btn-secondary w-full text-center block">{{ __('Back to Students') }}</a>
                    </div>
                </div>
                ...
                <div class="glass-card overflow-hidden" data-aos="fade-left" data-aos-delay="50">
                    <div class="glass-card-header">
                        <h3 class="font-bold" style="color: var(--color-text);">{{ __('Grant Free Access') }}</h3>
                    </div>
                    <div class="glass-card-body">
                        @if($availableCourses->isEmpty())
                            <p class="text-sm text-center" style="color: var(--color-text-muted);">{{ __('The student is already enrolled in all active courses.') }}</p>
                        @else
                            <form action="{{ route('admin.students.grant-access', $student) }}" method="POST" class="space-y-3">
                                ...
                                <button type="submit" class="btn-primary ripple-btn w-full" onclick="return confirm('{{ __('Grant free access to this course for the student?') }}')">
                                    {{ __('Grant Access') }}
                                </button>
                            </form>
                        @endif
                ...
                @if($student->is_telegram_linked)
                    <div class="glass-card overflow-hidden" data-aos="fade-left" data-aos-delay="100">
                        <div class="glass-card-header">
                            <h3 class="font-bold" style="color: var(--color-text);">{{ __('Send Telegram Message') }}</h3>
                        </div>
                        <div class="glass-card-body">
                            <form action="{{ route('admin.students.send-message', $student) }}" method="POST" class="space-y-3">
                                @csrf
                ...
                <div class="glass-card overflow-hidden" data-aos="fade-left" data-aos-delay="200">
                    <div class="glass-card-header">
                        <h3 class="font-bold" style="color: var(--color-text);">{{ __('Streaks') }}</h3>
                    </div>
