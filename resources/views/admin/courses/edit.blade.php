                    ...
                    <div>
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="payment_type" value="full" class="accent-primary-500" {{ old('payment_type', $course->payment_type ?? 'full') === 'full' ? 'checked' : '' }}>
                                <span class="text-sm" style="color: var(--color-text);">{{ __('Single payment') }}</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="payment_type" value="installment" class="accent-primary-500" {{ old('payment_type', $course->payment_type ?? 'full') === 'installment' ? 'checked' : '' }}>
                                <span class="text-sm" style="color: var(--color-text);">{{ __('3 monthly installments') }}</span>
                            </label>
                        </div>
                            ...
                            <select id="prerequisite_course_id" name="prerequisite_course_id" class="input-glass @error('prerequisite_course_id') border-red-500 @enderror">
                                ...
                                @foreach($allCourses as $c)
                                    <option value="{{ $c->id }}" {{ old('prerequisite_course_id', $course->prerequisite_course_id) == $c->id ? 'selected' : '' }}>
                                        {{ $c->title }} @if($c->is_exam) ({{ __('Exam') }}) @endif
                                    </option>
                                @endforeach
                            </select>
