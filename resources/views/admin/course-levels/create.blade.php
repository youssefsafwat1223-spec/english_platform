...
@section('title', __('إضافة عنوان جديد'))
@section('content')
<div class="py-12 relative overflow-hidden">
    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 relative z-10">
        <div class="mb-8" data-aos="fade-down">
            ...
            <h1 class="text-3xl font-extrabold mt-4"><span class="text-gradient">{{ __('إضافة عنوان جديد') }}</span></h1>
            <p class="mt-2" style="color: var(--color-text-muted);">{{ $course->title }}</p>
        </div>

        <div class="glass-card overflow-hidden" data-aos="fade-up">
            <form action="{{ route('admin.courses.levels.store', $course) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="glass-card-body space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" required class="input-glass" placeholder="{{ __('مثل: التمهيد الأول - First Introduction') }}">
                        @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-semibold mb-2" style="color: var(--color-text);">{{ __('وصف العنوان') }}</label>
                        <textarea name="description" rows="3" class="input-glass" placeholder="{{ __('وصف قصير عن محتوى العنوان...') }}">{{ old('description') }}</textarea>
                    ...
                    <div>
                        ...
                        <input type="number" name="order_index" value="{{ old('order_index') }}" min="0" class="input-glass">
                        <p class="text-xs mt-1" style="color: var(--color-text-muted);">{{ __('اتركه فاضي عشان يتضاف في الآخر') }}</p>
                ...
                <div class="glass-card-footer flex justify-between">
                    ...
                    <button type="submit" class="btn-primary ripple-btn">{{ __('إضافة العنوان') }}</button>
                </div>
            </form>
        </div>
    </div>
