...
@section('title', __('إضافة مستوى جديد'))
@section('content')
<div class="py-12 relative overflow-hidden">
    ...
    <div class="max-w-3xl mx-auto sm:px-6 lg:px-8 relative z-10">
        <div class="mb-8" data-aos="fade-down">
            ...
            <h1 class="text-3xl font-extrabold mt-4"><span class="text-gradient">{{ __('إضافة مستوى جديد') }}</span></h1>
            ...
            <form action="{{ route('admin.courses.levels.store', $course) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="glass-card-body space-y-6">
                    <div>
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" required class="input-glass" placeholder="{{ __('مثل: المستوى الأول - Level 1') }}">
                        @error('title') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-semibold mb-2" style="color: var(--color-text);">{{ __('وصف المستوى') }}</label>
                        <textarea name="description" rows="3" class="input-glass" placeholder="{{ __('وصف قصير عن محتوى المستوى...') }}">{{ old('description') }}</textarea>
                ...
                <div class="glass-card-footer flex justify-between">
                    ...
                    <button type="submit" class="btn-primary ripple-btn">{{ __('إضافة المستوى') }}</button>
                </div>
            </form>
        </div>
    </div>
