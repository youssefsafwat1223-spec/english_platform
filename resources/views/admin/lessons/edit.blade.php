                ...
                <div class="glass-card-body space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex items-center">
                            ...
                            <label class="ml-2 text-sm" style="color: var(--color-text);">{{ __('Has listening exercise') }}</label>
                        </div>
                    </div>
                    ...
                    <div id="pronunciationOptions" class="hidden">
                        <div class="rounded-xl p-4 space-y-4" style="background: var(--color-surface-hover);">
                            ...
                            <div>
                                ...
                                <input type="text" name="pronunciation_sentence_1" class="input-glass" value="{{ old('pronunciation_sentence_1', $pronunciationExercise?->sentence_1) }}" placeholder="{{ __('Example: cake') }}">
                            ...
                            <div>
                                ...
                                <input type="text" name="pronunciation_sentence_2" class="input-glass" value="{{ old('pronunciation_sentence_2', $pronunciationExercise?->sentence_2) }}" placeholder="{{ __('Example: The cake tastes sweet.') }}">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold mb-2" style="color: var(--color-text);">{{ __('Passage') }}</label>
                                <input type="text" name="pronunciation_sentence_3" class="input-glass" value="{{ old('pronunciation_sentence_3', $pronunciationExercise?->sentence_3) }}" placeholder="{{ __('Example: A short paragraph related to the lesson.') }}">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold mb-2" style="color: var(--color-text);">{{ __('Lesson Vocabulary') }}</label>
                                <textarea name="pronunciation_vocabulary_lines" rows="6" class="input-glass" placeholder="{{ __('One item per line: word | pronunciation | Arabic meaning') }}">{{ $existingVocabularyLines }}</textarea>
                                ...
                                @foreach ([1 => 'Word', 2 => 'Sentence', 3 => 'Passage'] as $audioIndex => $audioLabel)
                                    @php
                                        ...
                                        $audioTitle = app()->getLocale() === 'ar'
                                            ? match ($audioIndex) {
                                                1 => 'Reference Audio for Word',
                                                2 => 'Reference Audio for Sentence',
                                                default => 'Reference Audio for Passage',
                                            }
                    ...
                    <div id="listeningOptions" class="hidden">
                        <div class="rounded-xl p-4 space-y-4 border border-accent-200 dark:border-accent-500/20" style="background: var(--color-surface-hover);">
                            <h4 class="font-extrabold text-sm flex items-center gap-2" style="color: var(--color-text);">
                                {{ __('Listening Exercise Setup') }}
                            </h4>
                            <div>
                                <label class="block text-sm font-semibold mb-2" style="color: var(--color-text);">{{ __('Exercise Title') }}</label>
                            ...
                            <div>
                                <label class="block text-sm font-semibold mb-2" style="color: var(--color-text);">
                                    {{ __('Arabic TTS Script (audio)') }}
                                    <span class="text-xs font-normal ms-2" style="color: var(--color-text-muted);">{{ __('Use') }} <code>&lt;lang xml:lang="en-US"&gt;word&lt;/lang&gt;</code> {{ __('for English words.') }}</span>
                                </label>
                    ...
                    <div id="writingOptions" class="hidden">
                        <div class="rounded-xl p-4 space-y-4" style="background: var(--color-surface-hover);">
                            <div>
                                ...
                                <input type="text" name="writing_title" class="input-glass" value="{{ old('writing_title', $writingExercise?->title) }}" placeholder="{{ __('Example: Daily Routine Writing Task') }}">
                            ...
                            <div>
                                ...
                                <textarea name="writing_prompt" rows="4" class="input-glass" placeholder="{{ __('Write 80 to 120 words about your daily routine.') }}">{{ old('writing_prompt', $writingExercise?->prompt) }}</textarea>
                            ...
                            <div>
                                ...
                                <textarea name="writing_instructions" rows="3" class="input-glass" placeholder="{{ __('Use complete sentences and include clear supporting details.') }}">{{ old('writing_instructions', $writingExercise?->instructions) }}</textarea>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <input type="number" name="writing_min_words" class="input-glass" min="1" value="{{ old('writing_min_words', $writingExercise?->min_words ?? 30) }}">
                                ...
                                <div>
                                    ...
                                    <input type="number" name="writing_max_words" class="input-glass" min="1" value="{{ old('writing_max_words', $writingExercise?->max_words ?? 180) }}">
                                    @error('writing_max_words')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                                </div>
                                <div>
                                    <label class="block text-sm font-semibold mb-2" style="color: var(--color-text);">{{ __('Passing Score (%)') }}</label>
                                    <input type="number" name="writing_passing_score" class="input-glass" min="0" max="100" value="{{ old('writing_passing_score', $writingExercise?->passing_score ?? 70) }}">
                            ...
                            <div>
                                ...
                                <textarea name="writing_model_answer" rows="5" class="input-glass" placeholder="{{ __('Optional: add a strong sample answer for internal guidance or future display.') }}">{{ old('writing_model_answer', $writingExercise?->model_answer) }}</textarea>
                    ...
                    <div class="space-y-4">
                        <label class="block text-sm font-semibold" style="color: var(--color-text);">{{ __('Attachments') }}</label>

                        {{-- Existing Attachments --}}
                        @if($lesson->attachments && $lesson->attachments->count() > 0)
                        <div class="space-y-2">
                            <p class="text-xs font-semibold" style="color: var(--color-text-muted);">{{ __('Current attachments:') }}</p>
                            ...
                            <div class="flex items-center justify-between gap-3 p-3 rounded-xl" style="background: var(--color-surface-hover); border: 1px solid var(--color-border);">
                                ...
                                <div class="flex items-center gap-2 shrink-0">
                                    <a href="{{ Storage::url($attachment->file_path) }}" class="text-primary-500 text-xs font-bold hover:underline" download>{{ __('Download') }}</a>
                        ...
                        <div>
                            <label for="attachments" class="block text-xs font-semibold mb-1" style="color: var(--color-text-muted);">{{ __('Add new attachments (PDF, DOC, images, etc.)') }}</label>
